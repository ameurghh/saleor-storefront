...
export const CreditCardTile: React.FC<IProps> = ({
  ...
  const footer = (
    <S.FooterContent>
      <div>
        <IconButton name="trash" onClick={onRemove} size={22} data-test="removeCardButton" />
      </div>
    </S.FooterContent>
  );
  return (
    <Tile header={header} footer={footer} data-test="creditCardTile" data-testId={last4Digits}>
      {content}
    </Tile>
  );
};
